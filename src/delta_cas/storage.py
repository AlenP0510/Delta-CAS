...
delta_cas.storage
=================
Low-level file I/O: cross-platform file locking, atomic JSON writes, path helpers.
"""
...
if sys.platform == "win32":
    ...
    class FileLock:
        """Exclusive file lock — Windows (msvcrt)."""
    ...
    class FileLock:
        """Exclusive file lock — POSIX (fcntl). Works across processes."""
        def __init__(self, path: str):
            self._path = path + ".lock"
            self._fh   = None
...
def atomic_write_json(path: str, data: dict) -> None:
    """Write JSON atomically: write to tmp, then os.replace (atomic on POSIX + NTFS)."""
